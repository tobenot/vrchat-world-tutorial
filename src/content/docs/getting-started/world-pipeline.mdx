...
Hierarchy 里那一串 GameObject 看起来像一堆「物体」，但保存到磁盘上的 `MyFirstWorld.unity` 文件，本质是一份 YAML：
...
2. **平台编译**。Shader 会被编译成对应图形 API 的字节码（PC 是 D3D11/12 + Vulkan，Quest 是 OpenGL ES + Vulkan），UdonSharp 脚本被编译成 Udon 字节码。这一步很慢，是 Build 时间的大头。
...
1. 同样先打成 `.vrcw`。
2. SDK 用登录账号时拿到的 token，把这个文件 `PUT` 到 VRChat 后端的对象存储（CDN，目前主要是 Cloudflare R2）。
...
3. 文件落地后，客户端内嵌的 Unity Runtime 解包 AssetBundle，把 mesh、material、shader 装进显存，把场景结构在内存里重建出来。这一步做的事和编辑器里按 Play 时做的事**几乎一样**。
...
所以**同一个世界**在 CDN 上其实存的是**多份文件**：PC 版、Android 版（Quest 用）、iOS 版。每一份都要在 Unity 里切到对应 Build Target 重新打一次包再上传。Quest 版还会强制砍 shader、砍实时光数量、砍贴图大小（第七部第 38 章会专门展开）。
