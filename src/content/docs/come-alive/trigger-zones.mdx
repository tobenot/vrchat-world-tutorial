...
| 做法 | 典型设置 | 体验 |
|------|----------|------|
| 全局 BGM | 场景里一个 `AudioSource`，`Play On Awake` 开，`Spatial Blend = 0` | 进世界就开始响，全场音量差不多 |
| 区域触发（本章） | Trigger + `OnPlayerTriggerEnter`，`AudioSource` 放在区域内，由脚本 `Play()` / `Stop()` | 跨过区域边界才响，离开才停 |
