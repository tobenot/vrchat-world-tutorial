...
VR 玩家进了别人世界，第一件事常常是伸手抓东西。桌上一个杯子、一支荧光棒、一把小剑，能拿起来挥几下、扔出去看会不会摔，整个空间立刻不一样。桌面玩家瞄准物体按一下 V，照样能抓。
...
`Orientation` 和 `Auto Hold` 留默认值就好。要做手枪、手电筒、剑这种「必须握在手里特定角度」的物体，把 `Orientation` 改成 `Gun` 或 `Grip`，再在物体上做一个空 GameObject 当锚点，拖进 `Exact Gun` 或 `Exact Grip` 槽，VRChat 就会用这个锚点对齐玩家手心。
