...
| 组件 | 作用 | 用在哪 |
| :--- | :--- | :--- |
| **Products（产品）** | 用户最终买到的实际权益 | 主要分两类：Udon Products（世界内功能 / 内容解锁）、Subscription Role Products（群组特殊角色和权限） |
| **Listings（上架）** | 一次性销售入口 | 按持有时长分三类：Instant（立即消耗）/ Temporary（限时持有）/ Permanent（永久持有），打包一个或多个 Products 售卖 |
| **Subscriptions（订阅）** | 周期性付费维持福利 | 月度订阅，绑定 Subscription Role Product，自动加入群组并获得指定角色 |
| **Stores（商店）** | 发现 + 购买入口 | World Stores 关联世界，主要卖 Listings；Group Stores 关联群组，主要卖 Subscriptions |

收入分成大致：平台（Steam/Meta）30%、VRChat 与支付处理方约 20%、创作者约 50%。提现门槛 30,000 earned VRChat Credits（约 150 美元，手续费前），通过 Tilia 走 PayPal，PayPal 通道 ~1.5% 手续费。Credits 兑换上有两个视角：用户购买价约 1 USD ≈ 120 Credits（即 100 Credits ≈ 0.83 USD），卖家提现价约 200 earned Credits ≈ 1 USD（含约 50% 综合抽成）。具体数字以官方 `creators.vrchat.com/economy/payout/` 当前版本为准。
...
| 平台 | 抽成 | 提现门槛 | 优势 | 劣势 |
| :--- | :--- | :--- | :--- | :--- |
| **VRChat Creator Economy** | 约 50%（Steam/Meta + VRChat 共拿 50%） | 30,000 earned Credits（约 150 美元） | 在世界内直接买，体验流畅；自动处理订阅 | 抽成高，提现门槛偏高 |
