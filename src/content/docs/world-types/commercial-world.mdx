...
estimatedMinutes: 25
chapterType: concept
summary: 2024 年起 VRChat 推出 Creator Economy，世界开发者可以通过订阅、商品、付费角色获得 VRChat Credits 收益。这一章讲清楚这套经济系统的组件、规则、抽成比例，并讨论怎么把商业元素融进世界而不让它显得廉价。
sidebar:
  order: 8
---

VRChat Creator Economy（VRChat 创作者经济）2024 年正式上线，2025 年起逐步铺开。它解决的是一件长期被讨论的事：在 VRChat 里让创作有可持续的回报路径，而非永远只能算兴趣。
...
| 组件 | 作用 | 用在哪 |
| :--- | :--- | :--- |
| **Products（产品）** | 用户最终买到的东西 | 分两类：Udon Products（世界内功能 / 内容解锁）、Subscription Role Products（群组特殊角色和权限） |
| **Listings（一次性购买）** | 把一个或多个 Products 打包售卖 | 三种持续期：Instant 即时一次性 / Temporary 限时 / Permanent 永久 |
| **Subscriptions（订阅）** | 周期性付费维持福利 | 含 Subscription Role Product，自动加入群组并获得指定角色 |
| **Stores（商店）** | 发现 + 购买入口 | World Stores 关联世界，Group Stores 关联群组 |

收入分成大致：平台（Steam/Meta）30%、创作者 50%、VRChat + 支付处理方 20%。提现门槛 30,000 VRChat Credits（约 150 美元），通过 Tilia 走 PayPal。
...
- **永久 Listing**：买了之后任何时间进 VIP 区都行（演唱会的 SVIP 票）。
- **临时 Listing**：买了之后限定时间内有效（一场限时活动的入场券）。
- **订阅**：定期举办活动的常驻 VIP 通道。
...
VRChat Credits 大约 200 Credits = 1 美元。Listings 最低 100 Credits（约 0.5 美元）。订阅价格通常 200 到 1000 Credits / 月。

价格定得太高（一个 Pickup 物品 1000 Credits）：根本没人买。

价格定得太低（订阅 100 Credits / 月）：抽成之后到手太少（200 \* 0.5 = 100 → 减 30% 平台 → 减 20% VRChat = 30 Credits 到手），不划算。

参考社区里的常见定价：

- 一次性 Listing：100 到 500 Credits（0.5 到 2.5 美元）。
- 月度订阅：500 到 1500 Credits（2.5 到 7.5 美元）。
- 大型主题包 Listing：1000 到 5000 Credits（5 到 25 美元）。
...
| **Booth.pm**（日本主流） | 约 5% | 低 | 抽成低，VRChat 圈子核心 | 需要导出资源到外部 |
...
- VRChat Creator Economy 由 Products / Listings / Subscriptions / Stores 四块组成，抽成约 50%，提现门槛 30,000 Credits。
