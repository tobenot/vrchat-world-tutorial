...
- `OnDeserialization()`：本机客户端收到服务器推过来的同步变量更新时触发。新玩家自己加入时是从「全空白」状态收到一份完整快照（snapshot，某一时刻所有同步字段当前值打包成的初始数据），所以会触发一次。已经在场的玩家不会因为有人加入而触发 `OnDeserialization`（除非有人改了同步字段）。
