...
::::tip[本章新名词]
- **FPS**：每秒画多少帧；VR 里太低会直接让人晕。
- **Draw Call / Batches / SetPass call**：CPU 让 GPU 画东西的几类指令数，通常越少越好。
- **Profiler**：Unity 的逐帧性能分析器，用来找真正慢在哪。
- **Batching / GPU Instancing**：把很多物体合成更少绘制指令的办法。
- **Occlusion Culling**：预先算好「被墙挡住的物体不用画」。
- **Overdraw / Fillrate**：同一个像素被反复画、GPU 填像素填到吃力。
- **GC**：垃圾回收，脚本临时对象太多时会卡一下。
::::
...
::::note[这些数字是经验预算]
表里这些上限是社区里常见的「不太会塌」的工作预算，不是 VRChat 官方公布的硬性阈值。VRChat 官方给出的可参照硬限制目前主要在 Quest 包体（≤ 100 MB）和 Quest 内容优化建议（详见第 39 章）；其他几列偏高/偏低都还能跑，只是体验差异。把这张表当起点，最终以多人 + 真实头像下的实测为准。
::::
...
- 把所有镜子的 `Reflect Layers` 从 `Everything` 改成只勾 `Player`（包括 PlayerLocal 和 Player），看 Batches 变化。
...
- [VRChat Creator Docs：Udon Networking Performance](https://creators.vrchat.com/worlds/udon/networking/performance/)
