...
`Mixed Lighting` 下的 `Lighting Mode` 有几个选项，影响 Mixed 灯怎么混合烘焙和实时：
...
2. **Lightmap 上有缝（UV 接缝处出现亮线 / 黑线）**。物体的 Lightmap UV 没展开。Project 面板选中模型 fbx，Inspector → `Model` 选项卡 → 勾上 `Generate Lightmap UVs`，重新烘。
