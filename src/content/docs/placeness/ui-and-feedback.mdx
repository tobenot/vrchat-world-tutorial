---
title: 34. UI、提示和反馈
description: VRChat 不支持 Screen Space Overlay，所有 UI 都得做在三维空间里。这一章用 World Space Canvas + TextMeshPro 把欢迎牌、按钮提示和状态反馈做扎实，再讲怎么在 VR 里把中文字体加进来。
difficulty: 新手
estimatedMinutes: 45
chapterType: hands-on
requiresSDK: true
summary: VRChat 世界的 UI 只能用 World Space Canvas，做在三维里。文字用 TextMeshPro，中文字体要自己生成 SDF Font Asset。这一章把欢迎牌、按钮提示、状态反馈三类 UI 各做一遍，给一份在 VR 里能看清的字号 / 距离参考。
sidebar:
  order: 4
---
...
VRChat 世界的 UI 跟一般 Unity 项目不同。一般 Unity 游戏里能用屏幕空间叠加（Screen Space Overlay）做 HUD，玩家看到的是平铺在屏幕上的二维元素。**VRChat 世界里不行**：屏幕空间 UI 跟玩家自己的 VR 菜单冲突，会被禁用。所有世界 UI 都得做成 World Space（三维空间里的物体），玩家的 VR 头显或桌面相机像看一块画板那样看它。

这是一个限制，也是一个特点。它逼着作者把信息做成「场景里真的存在的牌子 / 屏幕 / 标签」，比一堆飘在屏幕角落的图标更有「地方」感。
...
- [VRChat Creator Docs：Interactions and Triggers](https://creators.vrchat.com/worlds/udon/interactions/)
