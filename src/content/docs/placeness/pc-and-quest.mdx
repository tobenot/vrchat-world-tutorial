...
| Quest 3 | Snapdragon XR2 Gen 2 | Adreno 740 | 2023 |
| Quest 3S | Snapdragon XR2 Gen 2 | Adreno 740 | 2024 |

XR2 Gen 2 大约相当于一台中端手机的算力。VRChat 在 Quest 上跑双眼 90Hz 渲染，每只眼睛 1832×1920 起步。GPU 分到每帧的预算非常少。
...
- `VRChat/Mobile/Particles/...`：粒子系统专用。
- 几个 Toon 子分支。

世界 Shader 没有像 Avatar 那样的硬性白名单（PC 版可以自定义 Shader），但 Quest 版本上自定义 Shader 性能往往跑不动。**实务上世界也按 Mobile 系列走**。
...
发布世界时，VRChat 网站会显示这个世界已经构建过哪些平台。第一次发布是 PC 版，要再发一次 Android 版（第 44 章会展开发布流程）。两次发布的是**同一个世界**（同一个 Room Id），客户端按玩家所在平台自动选。
