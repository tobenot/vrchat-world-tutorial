---
title: 39. PC 和 Quest
description: VRChat 的 Quest 版是 Android 构建。Shader 必须用 VRChat Mobile 系列、纹理 1K 上限、世界包体 100 MB 上限、几乎不能用透明、烘焙光必须用 Subtractive 模式。在同一个工程里同时维护 PC 和 Quest 两份构建。
difficulty: 进阶
estimatedMinutes: 50
...
VRChat 官方文档里有时把这部分叫 Quest，有时叫 Android。两个词在 VRChat 语境里**完全是同一件事**：Quest 设备运行 Android 系统，VRChat 给 Android 平台编译的 apk 包就是 Quest 版。

未来 VRChat 也可能把这个 Android 构建用到其他 Android 设备（手机、其他 XR）。但目前实际上跑这个版本的只有 Meta Quest 系列。所以行业里说 Quest 版 = Android 版，没区别。

第 39 章的「Quest」一律指代 Android 平台构建。
::::
...
├─ ● Windows           （PC 构建）
├─ ○ Android           （Quest 构建）
└─ ○ iOS               （iOS 构建，2025 年 10 月正式上线，需要 SDK 3.7.6 或更高）
```
...
## 第二节：Quest 必须用 VRChat Mobile Shader

VRChat 在 Quest 版本上**强制限制 Shader**。允许的范围：
...
| 规矩 | 必须 / 建议 |
| :--- | :--- |
| 贴图最大 1K | 必须（实际能跑但内存压力大） |
| 单世界三角面 ≤ 50,000 | 建议（给玩家头像留预算） |
...
- 只 PC：旁边一个红色 X，进不去。
- 只 Quest：可以进（PC 玩家也能进 Quest 版世界，反过来不行）。

发布世界时，VRChat 网站会显示这个世界已经构建过哪些平台。第一次发布是 PC 版，要再切到 Android 平台发一次 Quest 版（第 44 章会展开发布流程）。如果还要支持 iOS（VRChat iOS 客户端 2025 年 10 月正式上线），再切到 iOS 平台发一次。三次发布的是**同一个世界**（同一个 Room Id，绑定同一个 Blueprint），客户端按玩家所在平台自动选对应版本。
...
- 场景规模上，Quest 上一个 50,000 三角面的世界已经接近上限。所以世界范围别太大，单个房间面积控制在 30m × 30m 内，多个房间靠走廊或传送连。
