...
但 VRChat 客户端做了一件事：所有 Unity Audio Source 进入运行时之后会被 VRChat 接管，**统一通过 VRC Spatializer**（VRChat 的空间化算法）来播放。这一层接管包括两件事：

1. 默认套用一个反平方衰减曲线（更接近现实物理，比 Unity 默认对数曲线稍有不同）。
2. 给世界 Audio Source 默认加 `+10 dB` 的增益（Avatar Audio Source 没有这个加成）。
