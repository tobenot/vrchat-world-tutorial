...
🔀 **不同世界不同活法：** 休闲世界可能有几十盏装饰灯 → Prefab 让你一次调好全部氛围。游戏世界可能有几十个复活点 → Prefab 保证每个复活点的配置一致。展览世界可能有几十个画框 → Prefab 让你统一调整画框样式。
...
⚠️ **坑：** 正式项目里 Unpack 要谨慎，因为一旦断开就失去了统一更新的能力。如果以后想改所有同类物体的某个参数，Unpack 掉的那个就得单独手动改。
...
下一章，我们来看材质和光。你会发现，同样一块地板和几面墙，换一下颜色和光照方向，整个空间的感觉就完全变了。
