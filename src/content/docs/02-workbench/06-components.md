...
每个组件只管一件事，这是 Unity 的「单一职责」。所以你可以只要视觉不要碰撞（天上飘着的装饰云），也可以只要碰撞不要视觉（隐形的空气墙）。
...
举例：你写了一个按钮脚本，脚本上有一个 `Target Light` 字段。点按钮的时候要控制哪盏灯，脚本不知道，你必须把 Hierarchy 里那盏具体的灯拖进这个空槽。这叫显式指定引用。
...
为什么不让 Unity 自动找？因为场景里可能有十盏灯，引擎不知道你想控制哪一盏。手动拖几秒钟，逻辑就绝对清晰：一盏灯归一个开关，加灯不会影响已有逻辑。
:::
...
Unity 顶部那个三角形按钮是 **Play Mode**，按下去场景会在编辑器里运行起来。在里面你可以随便改参数看效果：灯不够亮拉 Intensity，声音吵就调 Volume，画面立刻响应。
...
Play Mode 下界面只是稍微变暗，新手很容易没注意。如果你调了半小时退出后发现全没了，多半是这个原因。
...
:::note[不同世界不同活法]
做什么类型的世界，决定你跟哪些组件打交道。休闲世界主力是 Mirror、Spatial Audio 和灯光；游戏世界主力是 Pickup、Object Sync 和 Udon 脚本。一开始不用全学，做到哪用到哪。
:::
