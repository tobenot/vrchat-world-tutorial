---
title: 7. Prefab，把东西做成模具
description: 理解 Prefab、Prefab Instance、Override，用它整理可复用的世界物件。
sidebar:
  order: 3
---

## 没有 Prefab 是什么样

你做了一个发光球。颜色调好了，Light 的 Range 设成 5，Intensity 设成 2，看着不错。

然后你在房间另外三个角落各放一个，`Ctrl+D` 复制三份拖到位。四个球了。

半小时后你觉得 Range 5 太小，照不亮房间，要统一改成 8。于是逐个点四个球，改四次。

四个还行。再想象一下：赛博朋克走廊，两侧二十盏壁灯，要统一微调霓虹颜色。改二十次。改到第十五盏你就会想：能不能改一次，所有同类灯一起变？

能。这就是 Prefab。

## 制作模具

Prefab 大致可以理解为「生产模具」。你配好一个物体，把它存成模具。以后从模具里压出新实例，配置完全一致。

操作两步：

1. 在 Project 面板的 `Assets` 下新建文件夹 `Prefabs`。
2. 把 Hierarchy 里配好的 `GlowBall` 直接拖进 `Prefabs` 文件夹。

拖完会看到两个变化：Project 面板里多了一个蓝色的 `GlowBall.prefab` 文件；Hierarchy 里 `GlowBall` 的名字和图标变成了蓝色，它现在是一个 Prefab Instance（预制体实例），和磁盘上那个模具绑定了。

:::tip[为什么是这样？]
Prefab 的核心思路是「单一数据源」：二十盏灯的配置只存一份在 `.prefab` 文件里，每个实例只记录自己的位置和跟模具不同的地方。改模具一次，全场同步。

如果不这样，二十盏灯里改第十九盏漏了，进 VR 一看那一盏颜色不对，往回查就成了体力活。
:::

## 从模具压出实例

把 Project 面板里的 `GlowBall.prefab` 拖进场景，每拖一次多一个配置好的球。拖三次，加上原来那个，四个球。

## 修改模具，全场同步

双击 Project 面板里的 `GlowBall.prefab`，进入 Prefab 编辑模式（四周变暗，只剩这个球）。

把 Light 的 Range 从 5 改成 8，`Ctrl+S` 保存，点左上角箭头退出。

回到场景，四个球的灯光范围全都自动变成了 8。改一次，全部生效。

## Override：允许个别实例不一样

二十盏灯都一样，但你想让其中一盏亮绿色。

选中场景里的 `GlowBall_C`，直接把 Light 颜色改成绿色。改完后这个字段旁边出现一条蓝色小标记，Inspector 右上角会多出 `Overrides` 下拉，这代表这个实例在这个参数上和模具不同了。

接下来：改模具的 Range，`GlowBall_C` 的 Range 跟着变（没被 Override）；改模具的颜色，`GlowBall_C` 颜色不变（已被 Override）。

:::caution[Override 多了会乱]
Override 太多，实例和模具的关系会变得难以追踪。一个实例上蓝条多到你自己都看不懂时，说明它已经偏离模具太远。这时把它做成新的独立 Prefab，或者用 Prefab Variant（预制体变体）来管，比堆 Override 干净得多。
:::

## 什么时候该做成 Prefab

判断法则很简单：发现自己要复制同一组配置两次以上，就该做成 Prefab。

门（模型 + 碰撞体 + 音效 + 脚本）、交互按钮、椅子、篝火、路灯、传送点，凡是有重复性质的物件，刚做出来就顺手做成 Prefab。等到第十次复制时再来反悔，已经晚了。

## Unpack：把实例和模具断开

右键一个 Prefab 实例，选 `Unpack Prefab`，它的名字会从蓝色变回黑色。此后模具怎么改都不再影响它。

适合你引入了别人的 Prefab 想拆开研究的时候。但在自己的项目里慎用，解包后就失去全局一键更新的能力。

## 项目目录结构

随着项目推进，文件会越来越多。从第一天就建好这个目录：

```text
Assets/
├─ Scenes/       场景文件
├─ Prefabs/      预制体
├─ Materials/    材质
├─ Audio/        音频
└─ Scripts/      脚本
```

不用追求精细，但分类要有。一周后你会感谢现在多花的两分钟。
