...
> 字段以当前 SDK / Unity 2022.3 LTS 的 Inspector 显示为准。SDK 升级后新增 / 删除字段时，以官方 [SDK Components 文档](https://creators.vrchat.com/worlds/components/) 当前页面为准。
...
通用字段 `Is Trigger`：勾上 = 不阻挡只检测；不勾 = 物理阻挡 + 碰撞事件。要用 `OnPlayerTriggerEnter` 时，`Is Trigger` 必须勾上（[第 14 章](/come-alive/trigger-zones/)）。
...
下面这几个本书正文还没单独详写，等到的时候再补到这里：

- Avatar Pedestal（第 35 章顺带涉及，暂未单独详写）
- Light Probe Proxy Volume（大型动态物体取光，进阶）
- Cloth（第 38 章 / 第 39 章 Quest 性能强相关，可单列）

碰到这几个组件需要查字段时，先去官方 [SDK Components 文档](https://creators.vrchat.com/worlds/components/) 或 Unity Manual 对应页查。
