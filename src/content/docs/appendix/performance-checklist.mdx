...
Quest 是渲染预算大约只有中端 PC 1/10 的 Android 设备。下面这些是 [VRChat 官方 Quest 内容优化文档](https://creators.vrchat.com/platforms/android/quest-content-optimization/) 列的硬规矩。
...
- [ ] 所有 Material Shader 改成 `VRChat/Mobile/...` 系列：
  - `VRChat/Mobile/Lightmapped`（性能最好，配合 Subtractive 用）
  - `VRChat/Mobile/Bumped Diffuse`（带 Normal Map）
  - `VRChat/Mobile/Toon Lit`（最便宜的卡通着色）
  - `VRChat/Mobile/Standard Lite`（简化版 PBR）
  - `VRChat/Mobile/MatCap Lit`（卡通 / Anime 风）
