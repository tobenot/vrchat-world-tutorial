...
### H.4.7 Content Warnings 怎么填

::::caution[Content Warnings 不是免责标志]
SDK 上传面板里的 Content Warnings（`content_adult` / `content_gore` / `content_horror` / `content_violence` / `content_sex` / `content_combat` 等）目前仍然作为可选标记保留，玩家端会按设置过滤这些世界。但**勾上不等于「打了标就可以上」**：违反 Community Guidelines / ToS 的内容，无论有没有标记都会被处理。
::::
