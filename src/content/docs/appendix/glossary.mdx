...
**循环 `for` / `while`**  
把一段事重复做。`for` 适合「重复固定次数」，`while` 适合「直到某条件不成立才停」。UdonSharp 不支持 `foreach`。  
...
VRChat 提供的受限 URL 类型与字符串下载工具。世界要从外部地址读文本只能走这套通道，普通 `System.Net.*` 调不到。URL 还要在 SDK 控制面板里登记。  
...
拿着 Pickup 时按下 / 松开"使用键"（鼠标左键、扳机）的事件。可以用来做"按住喷射、松手停"这类交互。  
...
告诉镜子要画哪些 Layer。剔除 `UI`、`Water`、`PlayerLocal` 等层后镜子开销会显著下降。  
...
`VRC Scene Descriptor` 上决定从 Spawns 数组里挑哪个的策略。`First` 一直用第一个；`Random` 随机；`Sequential` 顺序轮；`Demo` 用第一个但允许 Vehicle 接管。  
