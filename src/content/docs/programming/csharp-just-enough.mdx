...
在拆变量、控制流之前，先把三件最朴素的语法物理过一遍。这三件不讲，新手会被一些莫名其妙的红字卡住。
...
| `Vector3` | 三维坐标 | `Vector3 spawnPoint = new Vector3(0, 1, 0);` |
...
注意「相等」是两个等号 `==`，单个等号 `=` 是赋值。这是新手最常踩的笔误之一。
...
按下 Use 键之后，Unity 顶上的菜单 `Window → General → Console` 打开的那个面板里会看到刚才打出来的字。这是新手最有效的「程序到底跑没跑、跑到哪一步、变量是什么值」的检查手段。
...
写脚本时推荐一个很短的循环：写 5 到 10 行，保存，看 Console 有没有红字；没有红字，再进 Play 模式或 Build & Test 试一次。不要一口气写五十行再运行。报错越早出现，现场越干净，原因越容易找。
